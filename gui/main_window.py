...
from PyQt6.QtWidgets import (QMainWindow, QWidget, QHBoxLayout, QVBoxLayout,
                             ...
                             QTextBrowser, QDialog, QMenu, QSizePolicy, QSizeGrip)
...
from gui.widgets.dialog import FramelessDialog, MessageDialog

from core.engine import PivotEngine, EngineState
from core.entities import EntityType, ENTITY_META, DEPTH_LABELS, Depth
from core.risk import analyze
...
class MainWindow(QMainWindow):
    ...
    def _topbar_wrap(self, bar: QWidget) -> QWidget:
        ...
        v.addWidget(bar)
        # persistent authorized-scope banner
        banner = QFrame(); banner.setObjectName("banner"); banner.setFixedHeight(28)
        bl = QHBoxLayout(banner); bl.setContentsMargins(14, 2, 14, 2)
        self._banner_text = QLabel("")
        self._banner_text.setObjectName("bannerText")
        bl.addWidget(self._banner_text); bl.addStretch()
        v.addWidget(banner)
        self._refresh_banner()
        return wrap

    def _refresh_banner(self) -> None:
        mode = "ACTIVE + PASSIVE" if self._gate.active_scan else "PASSIVE ONLY"
        self._banner_text.setText(
            f"● AUTHORIZED SCOPE: {self._gate.target}  (apex {self._gate.apex})"
            f"   ·   {mode}   ·   ORG INFRASTRUCTURE ONLY — NO PERSON TARGETING")
    ...
    def _build_central(self) -> None:
        ...
        hint = QLabel("Click a node to trace its exposure path from the seed · "
                      "hover an edge for the relationship · node size grows with "
    ...
    def _build_left_dock(self) -> None:
        ...
        self._add_dock("INTELLIGENCE", split, Qt.DockWidgetArea.LeftDockWidgetArea, 300)
